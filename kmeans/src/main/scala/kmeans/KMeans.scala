...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var result : TrieMap[Point, ArrayBuffer[Point]] = new TrieMap[Point, ArrayBuffer[Point]]()
    if (points.isEmpty) {
      if (!means.isEmpty) {
        means.foreach { x => result.put(x, ArrayBuffer()) }
      }
    } else {
      points.foreach { p =>
        val m = findClosest(p, means)
        if (result.contains(m)) {
          val pts = result(m)
          result.put(m, pts :+ p)
        }
        else {
          result.put(m, ArrayBuffer(p))
        }
      }
    }
    result
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    val result = new Array[Point](oldMeans.length)
    for (i <- (0 until oldMeans.length)) {
      val p = oldMeans(i)
      result(i) = findAverage(p, classified(p))
    }
    result
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    var isConverged : Boolean = true
    for (i <- (0 until oldMeans.length) if (isConverged)) {
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    if (converged(eta)(means, updated)) {
      ...
      kMeans(points, updated, eta) // your implementation need to be tail recursive
    }
  }
}
